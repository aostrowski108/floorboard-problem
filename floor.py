...
def calculate_total_ways(conflict_list, height):
    ...
    possible_ways = [1] * num_rows  # Base case: 1 way to build the floor with 1 row

    # Build the solution for each height from 2 to the given height
    for _ in range(1, height):
        new_possible_ways = [0] * num_rows
        for i in range(num_rows):
            for j in conflict_list[i]:
                new_possible_ways[j] += possible_ways[i]
        possible_ways = new_possible_ways

    return sum(possible_ways)  # Sum of all ways for the topmost row
